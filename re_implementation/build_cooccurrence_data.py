...
def run_cooccurrence_calculation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with multiprocessing.Pool(processes=NUM_PROCESSES) as pool:
            ...
            for _ in tqdm(pool.imap_unordered(process_chunk_and_save, tasks_generator), total=total_tasks,
    ...
    # --- Pruning, Visualization, and Saving ---
    logger.info(f"Pruning pairs with count < {MIN_COOCCURRENCE_COUNT}...")
    pruned_counts = Counter(
        {pair: count for pair, count in final_cooccurrence_counts.items() if count >= MIN_COOCCURRENCE_COUNT})
